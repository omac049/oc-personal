...
- The Signal redesign (black/white/signal-green palette, DM Serif Display + Inter, signal-O mark, fixed top nav, generative hero grid) is **fully implemented** as of 2026-05-06. Spec and plan live in `docs/superpowers/specs/2026-05-06-signal-rebrand-design.md` and `docs/superpowers/plans/2026-05-06-signal-rebrand-design.md`. Active components: `SignalLogo`, `Navbar`, `SignalGrid`, `CountUp`, `Hero`, `ProofStrip`, `Services`, `CaseStudies`, `About`, `FAQ`, `Contact`, `Footer`. Three stale components remain in `src/components/` but are unused: `InsightsSection.tsx`, `ProofSection.tsx`, `ServicesOverview.tsx`.
